...
int mem_cfg = 0;  // EEPROM address where we store last used EPROM information (value 1 = 2708 2 = 2716, 3 = 2732, 4 = 2764, 5 = 27128, 6 = 27256, 7 = 27512)
...
const char FW_ver[] = "2.0rc4";
char HW_ver[10];

const char* memTable[8] = {"27xx","2708", "2716","2732","2764","27128","27256","27512"}; // lookup table for memory type
...
void setup() {
  ...
  setBus(7); //  default settings to enable all address lines  7 = 27512 memory code
  ...
  if ((EEPROM.read(mem_cfg) > 7) || (EEPROM.read(mem_cfg) < 1)) {
    EEPROM.write(mem_cfg,7);  // set default to 27512
  ...
  if(HW_ver[0] != 'v') {
    ...
    //
    EEPROM.put(ver_cfg, "v2.2d"); //Set this to whatever you have on the PCB, if you did the HW mod as per Errata, call it v1.6f or similar   <--------- README                                  
...
void load_SPIEEPROM (int eprom) {
  ...
  setBus(7); // allows access to enire 64kb SRAM Memory - equivlet of emulating 27512 eprom
  ...
  switch (eprom) {
    case 1:
      Serial.println(F( "-> Loading 2708 EPROM from SPI"));
      max_size = 8; // pages of 128 bytes
      break;
    case 2:
...
//  Adjusts address lanes A11 - A15, based on imput EPROM type is 2708 = 1 2716 = 2 ... 27512 = 7

void setBus (int eprom){
  switch (eprom) {
    case 1:
      Serial.println(F( "-> Bus set to 2708 EPROM Memory"));
